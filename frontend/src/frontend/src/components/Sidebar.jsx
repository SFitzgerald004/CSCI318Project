...
export default function Sidebar({ tripName }) {
  ...
  const isActive = (path) => {
    // Recommendations has nested routes (/recommendations/:recId), so match by prefix
      ...
      {id ? (
        <>
          ...
          <nav className="flex flex-col gap-1">
            ...
            <Link to={`/trips/${id}/itinerary`} className={linkClass(`/trips/${id}/itinerary`)}>
              <MapIcon className="w-4 h-4" /> Itinerary
            </Link>
            <Link to={`/trips/${id}/flights`} className={linkClass(`/trips/${id}/flights`)}>
              <GlobeAltIcon className="w-4 h-4" /> Flights
            </Link>
          </nav>
        </>
      ) : (
